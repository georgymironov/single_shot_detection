...
def resize(target, new_w, new_h, width, height):
    ...
    target[..., 0].clip(min=0, out=target[..., 0])
    target[..., 1].clip(min=0, out=target[..., 1])
    target[..., 2].clip(max=new_w - 1, out=target[..., 2])
    target[..., 3].clip(max=new_h - 1, out=target[..., 3])
...
def rotate(target, width, height, angle):
    ...
    target[..., 0].clip(min=0, out=target[..., 0])
    target[..., 1].clip(min=0, out=target[..., 1])
    target[..., 2].clip(max=width - 1, out=target[..., 2])
    target[..., 3].clip(max=height - 1, out=target[..., 3])
...
def crop(target, xmin, ymin, width, height, min_iou=0.5, keep_criterion='center_point', min_objects_kept=1):
    ...
    if jaccard.max() > min_iou:
        ...
        new_target[..., [0, 2]] -= xmin
        new_target[..., [1, 3]] -= ymin
        new_target[..., 0].clip(min=0, out=new_target[..., 0])
        new_target[..., 1].clip(min=0, out=new_target[..., 1])
